...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Guptacoin " + FormatFullVersion();
...
void ThreadOpenConnections()
{
    ...
    while (true)
    {
        ...
        if (addrman.size() == 0 && (GetTime() - nStart > 60)) {
            ...
            if (!done) {
                LogPrintf("No addresses available. Use -connect to connect to a node.\n");
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Guptacoin Core is probably already running."), addrBind.ToString());
        else
